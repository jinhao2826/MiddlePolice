...
char mbox_ip[15] = "192.168.150.3";
char victim_ip[15] = "192.168.160.4";
...
static unsigned int hook_func_in(void *priv, struct sk_buff *skb, const struct nf_hook_state *state){
	...
	//Strip the outer IP/UDP header. In some case, the linux kernel will strip packets for us
...
int init_module()
{
    ...
    for_each_net(initial_net){
    	result = nf_register_net_hook(initial_net, &nf_hook_in);
    	if(result != 0) {
   	    printk(KERN_INFO "Init_module: cannot register Netfilter hook at NF_INET_PRE_ROUTING\n");
            return -1;
    	}
    }

    for_each_net(initial_net){
    	result = nf_register_net_hook(initial_net, &nf_hook_out);                    
    	if(result != 0) {
    	    printk(KERN_INFO "Init_module: cannot register Netfilter hook at NF_INET_POST_ROUTING\n");
            return -1;
        }
    }    
    return 0;                                    
...
void cleanup_module() {
    for_each_net(initial_net){
	nf_unregister_net_hook(initial_net, &nf_hook_in);   
    }
    for_each_net(initial_net){
    	nf_unregister_net_hook(initial_net, &nf_hook_out);                
    }
}
